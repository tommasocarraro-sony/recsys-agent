...
1. A pre-trained recommendation system the LLM can access. In this project, we use a simple Matrix Factorization model trained on the MovieLens-100k dataset using the [RecBole]{https://recbole.io/} recommendation framework;
...
3. A [Qdrant]{https://qdrant.tech/} vector store containing embedded item descriptions that can be used by the LLM to answer to ambiguous or complex queries where the recommendation system is not enough to provide a comprehensive reply. This vector store is run inside a Docker container;
...
5. An API to make the LLM interact with the user-defined tools. This is also called the middleware layer. This layer has to detect function calls, call the correct function, and return the result back to the LLM. In our project, we use the [LangChain/LangGraph]{https://www.langchain.com/} framework.
